...
fun RecipeExecutor.armsRecipe(provider: ArmsPluginTemplateProviderImpl, data: ModuleTemplateData) {
    ...
    if (provider.needAdapter.value) {
        val modelFile = File(data.rootDir, "${fFmSlashedPackageName(provider.adapterPackageName.value)}/${provider.pageName.value}Adapter.$languageSuffix")
        save(armsAdapter(isKt, provider), modelFile)
    }

    if (provider.needEntity.value) {
        val modelFile = File(data.rootDir, "${fFmSlashedPackageName(provider.entityPackageName.value)}/${provider.pageName.value}Entity.$languageSuffix")
