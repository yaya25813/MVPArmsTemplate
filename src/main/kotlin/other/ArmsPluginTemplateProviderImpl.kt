...
class ArmsPluginTemplateProviderImpl : WizardTemplateProvider() {
    ...
    val needEntity = booleanParameter {
        name = "Generate Entity"
        default = true
        help = "是否需要生成 Entity ? 不勾选则不生成"
    }
    val entityPackageName = stringParameter {
        name = "Entity Package Name"
        constraints = listOf(Constraint.PACKAGE, Constraint.STRING)
        suggest = { "${appPackageName.value}.mvp.model.entity" }
        default = "${appPackageName.value}.mvp.model.entity"
        visible = { needEntity.value }
        help = "Entity 将被输出到此包下,请认真核实此包名是否是你需要输出的目标包名"
    }

    val needAdapter = booleanParameter {
        name = "Generate Adapter"
        default = true
        help = "是否需要生成 Adapter ? 不勾选则不生成"
    }
    val adapterPackageName = stringParameter {
        name = "Adapter Package Name"
        constraints = listOf(Constraint.PACKAGE, Constraint.STRING)
        suggest = { "${appPackageName.value}.mvp.ui.adapter" }
        default = "${appPackageName.value}.mvp.ui.adapter"
    ...
    val generateAdapterLayout = booleanParameter {
        name = "Generate Adapter Layout"
        default = true
        visible = { needAdapter.value }
        help = "是否需要给 Adapter 生成布局? 若勾选,则使用上面的布局名给此 Adapter 创建默认的布局"
    }
